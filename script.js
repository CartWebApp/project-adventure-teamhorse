...
const trueFinalBoss = new monster(`The True Sgriobhadair`,10,3,`It's emotions confuse you. You think he plans to ${() => {if(Math.random() > 0.3){return `attack`}else{return `defend`}}}`,`It's emotions confuse you. You think he plans to ${() => {if(Math.random() > 0.3){return `defend`}else{return `wait`}}}`,`It's emotions confuse you. You think he plans to ${() => {if(Math.random() > 0.3){return `wait`}else{return `attack`}}}`)
...
const secondShop = new storyPart(`9bb`,[``,`You decide to visit the store to stock up.`,`Shopkeeper: “Welcome to my shop! What are you looking for?”`],[{name: "Buy Health Potion: £2", effect: ()=> {if(player.money >= 2){player.inventory.healthP += 1; player.money -= 2}else{if(document.getElementById("textDisplay").textContent == `Shopkeeper: “Welcome to my shop! What are you looking for?”`){textType(document.getElementById("textDisplay"), `Shopkeeper: "I'm sorry, it appears you don't have enough money."`)};}; body.removeEventListener("click",advanceDialogue); update(); inventoryDisplay.style.opacity = 1;}},{name: "Buy Energy Potion: £2", effect: ()=> {if(player.money >= 2){player.inventory.energyP += 1; player.money -= 2}else{if(document.getElementById("textDisplay").textContent == `Shopkeeper: “Welcome to my shop! What are you looking for?”`){textType(document.getElementById("textDisplay"), `Shopkeeper: "I'm sorry, it appears you don't have enough money."`)};}; body.removeEventListener("click",advanceDialogue); update(); inventoryDisplay.style.opacity = 1;}},{name: "Leave Shop", effect: () => {if(player.companion == 1){moveTo(outpostChad)}else{moveTo(outpostOwain)}}}])
