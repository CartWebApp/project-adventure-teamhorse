...
const thirdMonster = new monster(`Ploobus`,6,0,`The Ploobus is hungry.`,`The Ploobus doesn't want to put in effort.`,`The Ploobus is just staring at you.`);
...
const finalBoss = new monster(`Sgriobhadair`,8,2,`Sgriobhadair's anger flares.`,`Sgriobhadair readies himself.`,`Sgriobhadair is thoughtful.`)
const trueFinalBoss = new monster(`The True Sgriobhadair`,10,3,`It's emotions confuse you. You think he plans to ${trueBossRandomness(`attack`)}.`,`It's emotions confuse you. You think he plans to ${trueBossRandomness(`defend`)}.`,`It's emotions confuse you. You think he plans to ${trueBossRandomness(`wait`)}.`)
...
const thirdFight = new fight(`A wild Ploobus approaches!`,thirdMonster,player.companion)
...
const beastFight = new fight(`You quiver in your boots as you face the Beast.`, beast,player.companion)
const bossFight = new fight(`You prepare for combat, ready for the fight of your life.`,finalBoss,player.companion)
const trueBoss = new fight(`You ready yourself to defeat your horse.`,trueFinalBoss,player.companion);
...
const fishing = new storyPart(`1aa`,[``,`You pull out your rod, ready your line, and prepare bait.`],[{name: `Cast your Rod`, effect: () => {document.getElementById(`textDisplay`).textContent = ``; fish()}},{name: `Leave the River`, effect: () => {moveTo(secondLeft); document.getElementById(`option#0`).style.opacity = 1; document.getElementById(`option#0`).style.zIndex = `auto`; }}]);
...
const wastelandFightChadWin = new storyPart(`8aa`, [``,`You Won`, `Chad von Chad: “Great work! It seems we can pick up the pace if you are this strong!”`, `You continue onward with newfound confidence`], [{name: "Continue deeper into the wastelands", effect: ()=> moveTo(outpostChad),}])
const wastelandFightChadLose = new storyPart(`8ab`, [``,`You Lost`, `Chad von Chad: “I expected more of you.”`, `Kameron (whispering): “Hmm.. should we have trained for longer?”`, `You are disappointed by your own weakness and follow the knights from behind as they move onwards.`], [{name: "Continue deeper into the wastelands", effect: ()=> moveTo(outpostChad),}])
...
const secondShop = new storyPart(`9bb`,[``,`You decide to visit the store to stock up.`,`Shopkeeper: “Welcome to my shop! What are you looking for?”`],[{name: "Buy Health Potion: £2", effect: ()=> {if(player.money >= 2){player.inventory.healthP += 1; player.money -= 2}else{if(document.getElementById("textDisplay").textContent == `Shopkeeper: “Welcome to my shop! What are you looking for?”`){textType(document.getElementById("textDisplay"), `Shopkeeper: "I'm sorry, it appears you don't have enough money."`)};}; body.removeEventListener("click",advanceDialogue); document.removeEventListener(`keydown`,advanceDialogue); update(); inventoryDisplay.style.opacity = 1;}},{name: "Buy Energy Potion: £2", effect: ()=> {if(player.money >= 2){player.inventory.energyP += 1; player.money -= 2}else{if(document.getElementById("textDisplay").textContent == `Shopkeeper: “Welcome to my shop! What are you looking for?”`){textType(document.getElementById("textDisplay"), `Shopkeeper: "I'm sorry, it appears you don't have enough money."`)};}; body.removeEventListener("click",advanceDialogue); document.removeEventListener(`keydown`,advanceDialogue); update(); inventoryDisplay.style.opacity = 1;}},{name: "Leave Shop", effect: () => {if(player.companion == 1){moveTo(outpostChad)}else{moveTo(outpostOwain)}}}])
const pubChad = new storyPart(`9ac`, [``,`You sit at the pub and visit with the people there.`, `One person in particular caught your attention.`,`He introduced himself as Owain, a magical detective.`, `You talk with him over a drink and he even shows you a few party tricks before you guys say goodbye and go your separate ways.`], [{name: "Continue", effect: ()=> moveTo(outpostChad)}])
const leaveOutpostChad = new storyPart(`9ad`, [``, `You reunite with the knights and rest for the night.`, `After morning comes Chad pulls you aside.`, `Chad von Chad: “Take this, it is a sword of the highest quality.”`, `Chad von Chad:  “Once we continue the monsters will only get stronger and you will need it if you wish to survive.”`], [{name: "Leave Outpost", effect: ()=> {player.attack += 1; player.telepathy = 4; player.health = 5;moveTo(ruinedTownChad)}}])
...
const ruinedTownFightLose = new storyPart(`10aa`, [``,`You Lost!`, `Chad von Chad: “The fights are only going to get harder from here on. PULL IT TOGETHER!”`, `You are saddened by your own incompetence.`], [{name: "Continue", effect: ()=> moveTo(searchRuins)}])
const ruinedTownFightWin = new storyPart(`10ab`, [``,`You Won!`, `Chad von Chad: “HAHAHA! You are an amazing fighter."`,`Chad von Chad: "Once we return please consider joining us as a fellow knight.”`, `Chad's sincere praise boosts your confidence and you begin to consider joining the Kights of the Square Table.`], [{name: "Continue", effect: ()=> moveTo(searchRuins)}])
const youSuckEndingChad = new storyPart(`E3a`,[``,`Chad von Chad: “The fights are only going to get harder from here on. PULL IT TOGETHER!”`,`The knights look at you with pity and begin to whisper among themselves.`, `You, Kameron, and Chad's knights investigate the ruins and prepare to bury the bodies of the deceased.`, `During your investigation one of the knights discover a note written in dried blood.`, `The note reads: ‘BEWARE SGRIOBHADAIR, MASTER OF THE ACCURSED TOWER’`, `Chad von Chad: “It seems that we have found our target.”`, `Kameron: “It seems this Sgri-”`, `Kameron: "Sgior..."`, `Kameron: "Sgribadaba...."`, `It appears that Kameron can't quite pronounce the name so you help him out.`, `You (whispering to Kameron): “It’s pronounced s̪kɾiːvətɛɾ.”`, `Kameron: “Ahem!” “It seems Sgriobhadair is the source of these rampaging monsters.”`, `Chad von Chad: “So our destination is the tower in the distance.”`, `You turn to look at the ominous tower that only recently came into veiw.`, `Chad von Chad: “REST UP MEN! We will have to fight our hardest if we wish to put an end to all of this.”`, `Before you can leave to set up your tent Chad pulls you aside with Kameron.`, `Chad von Chad: "Guy, you need to head back."`, `You: "What?", 'Kameron looks equally as surprised as you, but he nods his head in agreement with Chad.`, `Kameron: "Chad's right, you have held back the group too much."`, `You: "B-but what about Korai!?!"`, `Kameron: "I will escort you back to town and reunite with the knights before the tower."`, `Kameron: "I promise to continue the search in your stead."`, `Chad von Chad: "Will you be able to catch back up with us Kameron?"`, `Kameron: "Haha, don't worry about me I am stronger than this old body lets on."`, `CHad von Chad: "Alright, once day breaks Kameron will escort you back Guy, then he will rendevous with us and continue teh journey from there."`, `You wish to refute the two men, but deep down you know you can't.`,`Deep down you know that you suck.`],[{name: `Return to town`, effect: () => moveTo(youSuckEndingChad2)}]);
const youSuckEndingChad2 = new storyPart(`E3aa`,[``,`You are escorted to town and told to wait at the Knight's Guild.`,`You wait.`, `And wait.`, `And wait.`, `And wait.`, `And wait.`, `Weeks have past and Chad and Kameron have yet to return.`, `You finally decide to act and prepare to return back to the wastelands.`, `You gather your things and finally arrive at the border of the wastelands.`, `You cross the border with determination, but are immediately ambushed by a single small monster and get mauled to death.`, `Later Kameron and Chad return, but they are unable to find you.`,`THE END`], [{name: `Restart Game`, effect: () => {player.inventory.egg = 10; reset();}},{name: `Restart at last fight`, effect: () => moveTo(ruinedTownChad)}, {name: 'Restart at the beginning of the Wastelands', effect: () => moveTo(wastelandDepartChad)}]);
const searchRuins = new storyPart(`11a`, [``, `After the fight you, Kameron, and Chad's knights investigate the ruins and prepare to bury the bodies of the deceased.`, `During your investigation one of the knights discover a note written in dried blood.`, `The note reads: ‘BEWARE SGRIOBHADAIR, MASTER OF THE ACCURSED TOWER’`, `Chad von Chad: “It seems that we have found our target.”`, `Kameron: “It seems this Sgri-”`, `Kameron: "Sgior..."`, `Kameron: "Sgribadaba...."`, `It appears that Kameron can't quite pronounce the name so you help him out.`, `You (whispering to Kameron): “It’s pronounced s̪kɾiːvətɛɾ.”`, `Kameron: “Ahem! It seems Sgriobhadair is the source of these rampaging monsters.”`, `Chad von Chad: “So our destination is the tower in the distance.”`, `You turn to look at the ominous tower that only recently came into veiw.`, `Chad von Chad: “REST UP MEN! We will have to fight our hardest if we wish to put an end to all of this.”`, `You, Kameron, and the knights set up camp in the ruined village and night falls.`], [{name: "Rest", effect: () => moveTo(questionJourneyChad)}])
...
const questionJourney2Chad = new storyPart(`11ac`,[``,`You: ‘Like, me and Korai were close, but at the end of the day he’s just a horse. Do I really have to fight this Sgriobhadair just to get a clue on where he is?’`],[{name: `Question it even more`, effect: () => moveTo(questionJourney3Chad)},{name: `Ignore your doubts`, effect: () => moveTo(restBeforeTowerChad)}]);
...
const questionJourney4Chad = new storyPart(`11ae`,[``,`You: ‘Even if I couldn’t find Korai at the end of the day he’s just a horse.’`],[{name: `Question it even more`, effect: () => moveTo(questionJourney5Chad)}]);
...
const maze2Chad = new storyPart(`16aa`,[``,`You continue right and are met with another three way split in the path.`,`You can go right, forward, or left`], [{name: `Go right`, effect: () => Fight(mazeFight)},{name: `Go Forward`, effect: () => moveTo(maze3Chad)}, {name: `Go left`, effect: ()=> Fight(mazeFight)}]);
...
const maze5Chad = new storyPart(`16ad`,[``,`You turn left again.`,`You can feel yourself reaching the end of the maze.`, `But, once agin you must decide.`, `You can go left, forward, or right`],[{name: `Go left`, effect: () => Fight(mazeFight)},{name: `Go forward`, effect: () => Fight(mazeFight)}, {name: 'Go right', effect: ()=> moveTo(maze6Chad)}]);
...
const bossChad = new storyPart(`18a`,[``,`You approach the large set of double doors and push them open with all your might.`,`Behind them stands another cloaked figure, but he is much stronger than any other cloaked figure you have seen before.`, `???: “I …. Am Sgriobhadair.”`, `Sgriobhadair: “You have bested me up until this point, BUT NO LONGER!”`, `Sgriobhadair summoned hundreds of monsters separating you from Chad and the knights.`, `Sgriobhadair: “I will deal with you first.”`],[{name: `Fight`, effect: () => Fight(bossFight)}]);
const bossWinChad = new storyPart(`19aa`,[``, `You finally beat Sgriobhadair. You feel a huge weight off your shoulders and take a look around.`, `Chad and the knights finished off the last of the monsters.`, `Chad von Chad: “Everything is over. We did it!”`, `From behind a pillar in the room you hear a faint whine.`, `You quickly look around and spot your best friend.`, `You enjoy your reunion with Korai while Chad and the knights celebrate and prepare to head back.`, `As you leave the tower you see the remnants of Kameron’s battle with the beast.`, `A huge corpse lay still, and Kameron is nowhere to be seen.`, `You silently thank your Mentor and head back with the knights.`], [{name: `Continue`, effect: () => moveTo(bossEndChad)}]);
const bossEndChad = new storyPart(`E7a`, [``,`2 years later.`,`You joined the knights and succeeded Chad as the commander.`, `Hailed as heroes you had to lead your men on many difficult missions, but none as hard as your first.`, `You enjoy the rest of your life as a hero and respected commander of the Knights of The Square Table.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`], [{name: `Restart Game`, effect: ()=> reset()}, {name: `Return to doors`, effect: () => moveTo(bossDoorsChad)}])
const bossLoseChad = new storyPart(`E8a`,[``,`You lose to Sgriobhadair and are helpless as Sgriobhadair goes to wipe out the exhausted knights.`,`You can only hear screams and the crushing of bones as Sgriobhadair battles the knights with his monsters.`, `Sgriobhadair returns to you and looks at you with pity.`, `Sgriobhadair: “You were strong, but what is strength compared to my army?”`, `Sgriobhadair orders his monsters to finish you and the last thing you see is his twisted smile.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`],[{name: `Restart Game`, effect: ()=> reset()}, {name: `Return to doors`, effect: () => moveTo(bossDoorsChad)}]);
const shedDoorChad = new storyPart(`17ab`,[``,`You approach the shed door`],[{name: `Open`, effect: () => moveTo(shedDoorOpenChad)},{name: `Go to the other door`, effect: () => moveTo(lavishBossDoorChad)}]);
const shedDoorOpenChad = new storyPart(`17ac`,[``,`You approach the shed door and notice a strange keyhole on it.`,`The key hole is awkwardly shaped like a fish.`],[{name: `Try to open`, effect: () => {if(player.inventory.fish == 1){moveTo(trueBossChad)}else{moveTo(shedDoorLockedChad)}}},{name: `Go to other door`, effect: () => moveTo(lavishBossDoorChad)}]);
...
const betrayalChad = new storyPart(`HorseAA`,[``,`???: “Welcome Guy.”`,`Before you sits a horse on a throne. He appears to know you, and you fear that you know why.`, `You: “Korai?”`, `Korai: “Yes, that is the name you gave me all those years ago.”`, `Korai: “But I prefer my true name: Sgriobhadair!”`, `!!!!!!!!!!`, `You: “But …. How?”`, `Sgriobhadair: “Didn’t you ever find it weird?”`, `Sgriobhadair: “The unnatural intelligence I displayed, bmy ability to remain in my prime despite living almost ten years longer than every other horse.”`, `Sgriobhadair: “Didn’t you ever have even a sliver of doubt even after everything you have seen that maybe this was all orchestrated by me GUY!?!?!?”`, `Sgriobhadair: “Of course I did not expect you to find my key so effortlessly, but I have prepared myself to kill you ever since.”`, `Chad von Chad: “Guy, what is going on?”`, `You: “That’s Korai, my horse.”`, `Chad von Chad: “And he says he’s behind everything that has been happening lately?”`, `You: “... yes.”`, `Chad takes a moment to process what is going on, but he quickly prepares for battle and steps forward.`, `Chad von Chad: “Then there is no time to hesitate, throw away what sentiment you have and prepare for battle Guy!”`, `Confused, you draw your sword and point it at Korai - neigh Sgriobhadair.`],[{name: `Fight`, effect: () => Fight(trueBoss)}]);
const trueBossLoseChad = new storyPart(`E10a`,[``,`You and the knights lose to Sgriobhadair and fall unconscious.`,`As you awaken you find yourselves unable to control your own body.`, `Sgriobhadair: “I was planning to slowly take control of who I needed over the next hundred years but you guys are quite the splendid surprise.”`, `You and the knights once again fall unconscious.`, `The next time you awake you are surrounded by flames and covered in blood.`, `You have slaughtered thousands while your body wouldn’t listen to you.`, `Under Sgriobhadair’s command you plunge the world into chaos.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`],[{name: `Restart game`, effect: () => reset()}]);
const trueBossWinChad = new storyPart(`E11a`,[``,`After a long battle you finally beat Sgriobhadair.`,`Sgriobhadair’s face scrunches in annoyance and frustration as he charges at you one last time while raising his magical power.`, `You: “HE'S GOING TO BLOW HIMSELF UP!”`, `You and the knights run to the door but you can’t leave.`, `Chad turns around and dashes to Sgriobhadair.`, `Chad wrestles Sgriobhadair to the ground and throws him as hard as he can against the wall.`, `Sgriobhadair breaks through the wall and falls to the ground below the tower, he blows up leaving no one harmed.`, `You take a moment to relive the good memories you and with Korai.`, `As you leave the tower you see the remnants of Kameron’s battle with the beast.`, `A huge corpse lay still, and Kameron is nowhere to be seen.`, `You silently thank your Mentor and head back with the knights.`, `You return to town and Chad offers you a position in the Knights of The Square Table, but you refuse.`, `You vow to never let something like this happen again under your watch and spend the rest of your days acting as the world’s hero and protector.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`],[{name: `Restart game`, effect: () => reset()}]);

// Owain's path below
const wastelandDepartOwain = new storyPart(`8b`, [``,`It’s been a week since you departed with Owain and Kameron to the Wastelands.`, `Faithful to its namesake, the Wastelands show no form of life besides the occasional monsters that cannibalize each other and feed off of unfortunate travelers.`, `Your small group of three have had a smooth journey so far, but everyone knew remained on guard.`, `"ROOOOAAAR!"`, `Stalking you from a distance a large monster appears.`, `Your vigilance paid off as everyone in the group remained calm.`, `Owain: “Prepare yourselves! He's big but we can handle it!”`,`A cloaked being appears to be leading the monster and approaches you with the monster for combat.`], [{name: "FIGHT!", effect: ()=> Fight(secondFight)}])
const wastelandFightOwainWin = new storyPart(`8ba`, [``,`You defeated the figure.`,`Owain: “We did well. I believe we can move faster if we are this strong.”`, `You and Owain continue onward with newfound confidence.`], [{name: "Continue deeper into the wastelands", effect: ()=> moveTo(outpostOwain),}])
const wastelandFightOwainLose = new storyPart(`8bb`, [``,`The figure stands above you, victorious.`,`Owain: “Darnit! I didn't want to have to use this so soon”`, `Owain pulled out a small palm sized ball from his bag and began to inscribe runes onto it.`, `When he finished his inscriptions he threw the ball at the monster before the small item suddenly burst into a thick cloud of smoke.`, `Owain: "RUN NOW!"`, `As you and Owain run away defeated Kameron follows you from behind.`, `Kameron (whispering): “Hmm.. should we have trained for longer?”`], [{name: "Continue deeper into the wastelands", effect: ()=> moveTo(outpostOwain),}])
const outpostOwain = new storyPart(`9b`, [``,`You, Owain, and Kameron continue forward and eventually arrive at an outpost that marks the entrance into the depths of the Wastelands.`, `The three of you split up and plan to meet back up before the sun sets.`, `You can go to the shop and restock items, visit the pub and relax over a nice drink, or wait for Owain and Kameron at the meeting location.`], [{name: "Shop", effect: ()=> moveTo(secondShop)}, {name: "Visit pub", effect: ()=> moveTo(pubOwain)}, {name: "Wait for Owain and Kameron", effect: ()=> moveTo(leaveOutpostOwain)}])
...
const questionJourney1Owain = new storyPart(`11ab`,[``,`You: ‘I mean I love Korai, but I might actually die if I continue forward.’`],[{name: `Question it even more`, effect: () => moveTo(questionJourney2Owain)},{name: `Ignore your doubts`, effect: () => moveTo(restBeforeTowerOwain)}]);
const questionJourney2Owain = new storyPart(`11ac`,[``,`You: ‘Like, me and Korai were close, but at the end of the day he’s just a horse. Do I really have to fight this Sgriobhadair just to get a clue on where he is?’`],[{name: `Question it even more`, effect: () => moveTo(questionJourney3Owain)},{name: `Ignore your doubts`, effect: () => moveTo(restBeforeTowerOwain)}]);
...
const questionJourney4Owain = new storyPart(`11ae`,[``,`You: ‘Even if I couldn’t find Korai at the end of the day he’s just a horse.’`],[{name: `Question it even more`, effect: () => moveTo(questionJourney5Owain)}]);
...
const maze2Owain = new storyPart(`16ba`,[``,`You continue right and are met with another three way split in the path.`,`You can go right, forward, or left`], [{name: `Go right`, effect: () => Fight(mazeFight)},{name: `Go Forward`, effect: () => moveTo(maze3Owain)}, {name: `Go left`, effect: ()=> Fight(mazeFight)}]);
...
const maze5Owain = new storyPart(`16bd`,[``,`You turn left again.`,`You can feel yourself reaching the end of the maze.`, `But, once agin you must decide.`, `You can go left, forward, or right`],[{name: `Go left`, effect: () => Fight(mazeFight)},{name: `Go forward`, effect: () => Fight(mazeFight)}, {name: 'Go right', effect: ()=> moveTo(maze6Owain)}]);
...
const mazeEnd1Owain = new storyPart(`E5b`,[``,`Despite all logic and common sense telling you to go forward, you go left.`,`Your irational decision confuses Owain, but he trusts your instincts as a telepath.`, `And so, you continue left`, `You walk and walk`, `And walk`, `And walk`, `And walk`, `And walk`, `Eventually, you begin to think that going left wasn't such a good idea.`, `You try to turn back, but you can't.`, `There is only darkness, and you have no idea where Owain is.`, `Perhaps he used his brain and headed back in order to go down the correct path.`, `You look around.`, `You see nothing.`, `You hear nothing.`, `You feel nothing.`, `Left alone with your thoughts you begin to drift away.`, `THE END`], [{name: `Restart game`, effect: () => reset()}]);
const mazeEnd2Owain = new storyPart(`E6b`,[``,`Despite all logic and common sense tellin you to go forward, you go right`,`Your irational decision confuses Owian, but he trusts your instincts as a telepath.`, `And so you continue right.`, `You walk down a dimly lit path until you reach a dead end.`, `You awkwardly look back at Owain before deciding to head back.`, `But before you can take two steps the floor gives out beneath you and you along with the Owain plunge into a pit filled with small spikes.`, `The spikes are small enough to do you no real harm, but they are laced with a neurotoxin.`, `Anxiety and dread begin to well up inside of you as your body slowly loses function.`, `You can feel every emotion all at once as your brain is being fried by the poison.`, `You and Owain succumb to a deep insanity before dying.`, `THE END`], [{name: `Restart game`, effect: () => reset()}]);
const bossDoorsOwain = new storyPart(`17b`,[``,`After clearing the maze you are met with two doors.`,`One is a lavish double door lined with gold and radiating a menacing aura from behind it.`,`The other appears to belong to a wooden shed.`],[{name: `Approach the lavish door`, effect: () => moveTo(lavishBossDoorOwain)},{name: `Approach the shed door`, effect: () => moveTo(shedDoorOwain)}]);
const lavishBossDoorOwain = new storyPart(`17ba`,[``,`You approach the large black and gold double door`],[{name: `Open`, effect: () => moveTo(bossOwain)},{name: `Go to other door`, effect: () => moveTo(shedDoorOwain)}]);
const bossOwain = new storyPart(`18b`,[``,`You approach the large set of double doors and push them open with all your might.`,`Behind them stands another cloaked figure, but he is much stronger than any other cloaked figure you have seen before.`, `???: “I …. Am Sgriobhadair.”`, `Sgriobhadair: “You have bested me up until this point, BUT NO LONGER!”`, `Sgriobhadair summoned a wall of fire seperating you from Owain. Owain looks intrigued by the wall.`, `Sgriobhadair: “I will deal with you first.”`],[{name: `Fight`, effect: () => Fight(bossFight)}]);
const bossWinOwain = new storyPart(`19ba`,[``,`You finally beat Sgriobhadair. You feel a huge weight off your shoulders and take a look around.`, `Owain manages to put out the fire.`, `Owain: “I can't believe its over.”`, `From behind a pillar in the room you hear a faint whine.`, `You quickly look around and spot your best friend.`, `You enjoy your reunion with Korai while Owain creates an omelet from the golden eggs.`, `As you leave the tower you see the remnants of Kameron’s battle with the beast.`, `A huge corpse lay still, and Kameron is nowhere to be seen.`, `You silently thank your Mentor and head back with Owain`], [{name: `Continue`, effect: () => moveTo(bossEndOwain)}]);
const bossEndOwain = new storyPart(`E7b`, [``,`2 years later.`,`You joined Owain and made it rich as a detective pair.`, `Hailed as heroes, you enjoy a booming business.`, `You enjoy the rest of your life as a hero and respected detective along with Owain.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`], [{name: `Restart Game`, effect: ()=> reset()}])
const bossLoseOwain = new storyPart(`E8b`,[``,`You lose to Sgriobhadair and are helpless as Sgriobhadair goes to annihilate the distracted Owain`,`You can only hear screams and the crushing of bones as Sgriobhadair battles Owain.`, `Sgriobhadair returns to you and looks at you with pity.`, `Sgriobhadair: “You were strong, but what is your strength compared to mine?”`, `Sgriobhadair orders his monsters to finish you and the last thing you see is his twisted smile.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`],[{name: `Restart Game`, effect: ()=> reset()}, {name: `Return to doors`, effect: () => moveTo(bossDoorsOwain)}]);
const shedDoorOwain = new storyPart(`17ab`,[``,`You approach the shed door`],[{name: `Open`, effect: () => moveTo(shedDoorOpenOwain)},{name: `Go to the other door`, effect: () => moveTo(lavishBossDoorOwain)}]);
const shedDoorOpenOwain = new storyPart(`17ac`,[``,`You approach the shed door and notice a strange keyhole on it.`,`The key hole is awkwardly shaped like a fish.`],[{name: `Try to open`, effect: () => {if(player.inventory.fish == 1){moveTo(trueBossOwain)}else{moveTo(shedDoorLockedOwain)}}},{name: `Go to other door`, effect: () => moveTo(lavishBossDoorOwain)}]);
const shedDoorLockedOwain = new storyPart(`17ad`,[``,`You try to open the shed door but it is locked`],[{name: `Find another way`, effect: () => moveTo(knowledgeGain1Owain)}, {name: `Go to the other door`, effect: () => moveTo(lavishBossDoorOwain)}]);
const knowledgeGain1Owain = new storyPart(`17be`,[``,`As you try to find another way past the door, Owain attempts to light the door on fire.`,`Nothing happened.`, `Owain: “It appears to be protected by high tier magic.”`],[{name: `Try to decipher the magic`, effect: () => moveTo(knowledgeGain2Owain)}, {name: `Stop and go to the other door`, effect: ()=> moveTo(bossDoorsOwain)}]);
const knowledgeGain2Owain = new storyPart(`17bf`,[``,`You try to decipher the magic, but to no avail.`,`You could try again.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain3Owain)}, {name: `Stop and go to the other door`, effect: ()=> moveTo(bossDoorsOwain)}]);
const knowledgeGain3Owain = new storyPart(`17bg`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain4Owain)}, {name: `Stop and go to the other door`, effect: ()=> moveTo(bossDoorsOwain)}]);
const knowledgeGain4Owain = new storyPart(`17bh`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain5Owain)}, {name: `Stop and go to the other door`, effect: ()=> moveTo(bossDoorsOwain)}]);
const knowledgeGain5Owain = new storyPart(`17bi`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain6Owain)}, {name: `Stop and go to the other door`, effect: ()=> moveTo(bossDoorsOwain)}]);
const knowledgeGain6Owain = new storyPart(`17bj`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain7Owain)}, {name: `Stop and go to the other door`, effect: ()=> moveTo(bossDoorsOwain)}]);
const knowledgeGain7Owain = new storyPart(`17bk`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`,`You now understand the meaning of life, but you have yet to decipher the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain8Owain)}]);
const knowledgeGain8Owain = new storyPart(`17bl`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain9Owain)}]);
const knowledgeGain9Owain = new storyPart(`17bm`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`],[{name: `Continue to try to decipher the magic`, effect: () => moveTo(knowledgeGain10Owain)}]);
const knowledgeGain10Owain = new storyPart(`17bn`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`,`Your knowledge and wisdom approaches omniscience.`],[{name: `choice2`, effect: () => moveTo(knowledgeGain11Owain)}]);
const knowledgeGain11Owain = new storyPart(`E9b`,[``,`Time passes and you feel yourself getting closer to deciphering the magic.`, `Hundreds of years pass and you find yourself understanding everything, but no closer to breaking past the door.`, `You finally give up and decide to enjoy your omniscience.`, `THE END`],[{name: `Restart game`, effect: () => reset}, {name: `Return to doors`, effect: ()=> moveTo(bossDoorsOwain)}]);
const trueBossOwain = new storyPart(`HorseB`,[``,`You examine the lock on the door and after thinking for a while you think to pull out the fish you got at the beginning of your journey.`,`You didn't think it would work, but to your surprise the fish glows and floats out of your hands.`, `The shed door opens and the fish disappears.`, `	Beyond the door you see something that shakes you to your very core.`],[{name: `Continue`, effect: () => moveTo(betrayalOwain)}]);
const betrayalOwain = new storyPart(`HorseBA`,[``,`???: “Welcome Guy.”`,`Before you sits a horse on a throne. He appears to know you, and you fear that you know why.`, `You: “Korai?”`, `Korai: “Yes, that is the name you gave me all those years ago.”`, `Korai: “But I prefer my true name: Sgriobhadair!”`, `!!!!!!!!!!`, `You: “But …. How?”`, `Sgriobhadair: “Didn’t you ever find it weird?”`, `Sgriobhadair: “The unnatural intelligence I displayed, my ability to remain in my prime despite living almost ten years longer than every other horse.”`, `Sgriobhadair: “Didn’t you ever have even a sliver of doubt even after everything you have seen that maybe this was all orchestrated by me GUY!?!?!?”`, `Sgriobhadair: “Of course I did not expect you to find my key so effortlessly, but I have prepared myself to kill you ever since.”`, `Owain: “Guy, what is going on?”`, `You: “That’s Korai, my horse.”`, `Owain: “And he says he’s behind everything that has been happening lately?”`, `You: “... yes.”`,`Owain: “Then there is no time to hesitate, let’s finish this.”`, `Confused, you draw your sword and point it at Korai - neigh Sgriobhadair.`],[{name: `Fight`, effect: () => Fight(trueBoss)}]);
const trueBossLoseOwain = new storyPart(`E10b`,[``,`You and Owain lose to Sgriobhadair and fall unconscious.`,`As you awaken you find yourselves unable to control your own body.`, `Sgriobhadair: “I was planning to slowly take control of who I needed over the next hundred years but you guys are quite the splendid surprise.”`, `You and Owain once again fall unconscious.`, `The next time you awake you are surrounded by flames.`,`Under Sgriobhadair’s command you plunge the world into chaos.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`],[{name: `Restart game`, effect: () => reset()}]);
const trueBossWinOwain = new storyPart(`E11b`,[``,`After a long battle you finally beat Sgriobhadair.`,`Sgriobhadair’s face scrunches in annoyance and frustration as he charges at you one last time while raising his magical power.`, `You: “HE'S GOING TO BLOW HIMSELF UP!”`, `You and Owain run to the door but you can’t leave.`, `Owain turns around and prepares his magic.`, `Owain picks Sgrioghadair up with his spell and throws him as hard as he can against the wall.`, `Sgriobhadair breaks through the wall and falls to the ground below the tower, he blows up leaving no one harmed.`, `You take a moment to relive the good memories you and with Korai.`, `As you leave the tower you see the remnants of Kameron’s battle with the beast.`, `A huge corpse lay still, and Kameron is nowhere to be seen.`, `You silently thank your Mentor and head back with the knights.`, `You return to town and Owain offers you a position as a detective, but you refuse.`, `You vow to never let something like this happen again under your watch and spend the rest of your days acting as the world’s hero and protector.`, `THE END`, `Game created by Cameron Jennings and Owen Kolbert.`],[{name: `Restart game`, effect: () => reset()}]);
...
function fightWin(storyFight){
    ...
    if(storyFight.monster.ending === 1){
        ...
        document.getElementById("textDisplay").textContent = ``;
        if(player.companion == 1){
            moveTo(trueBossWinChad); 
        }
        if(player.companion == 2){
            moveTo(trueBossWinOwain); 
        }
    } else if (storyFight.monster.ending === 4){
        document.getElementById("textDisplay").textContent = ``;
        if(player.companion == 1){
            moveTo(backToBeginningChad); 
        }
        if(player.companion == 2){
            moveTo(backToBeginningOwain); 
        }
    } else {
        document.getElementById("textDisplay").textContent = ``;
        player.money += 4;
        ...
        if (currentPart.order == `8b`){
            moveTo(wastelandFightOwainWin)
        }
        if (currentPart.order == `10a`){
            moveTo(ruinedTownFightWin)
        }
        if (currentPart.order == `10b`){
            moveTo(stoneTabletFightWin)
        }
    }
    update();
}
...
function fightLose(storyFight){
    ...
    if(storyFight.monster.ending === 1){
        ...
        if(player.companion == 2){
            moveTo(bossLoseOwain); 
        }
    } else if (storyFight.monster.ending === 3){
        document.getElementById("textDisplay").textContent = ``;
        if(player.companion == 1){
            moveTo(trueBossLoseChad); 
        }
        if(player.companion == 2){
            moveTo(trueBossLoseOwain); 
        }
    } else if (storyFight.monster.ending === 4){
        document.getElementById("textDisplay").textContent = ``;
        if(player.companion == 1){
        ...
        if (currentPart.order == `8b`){
            moveTo(wastelandFightOwainLose);
        }
        if (currentPart.order == `10a`){
            moveTo(ruinedTownFightLose);
        }
        if (currentPart.order == `10b`){
...
function egg(){
    ...
    inventoryDisplay.style.opacity = 0;
    document.getElementById(`fullPlayerInfo`).style.zIndex = -500;
    ...
    for(let i = 0; i < 10; i++){
        document.getElementById(`egg${i}`).addEventListener(`click`, (e) =>{
            ...
            if(player.inventory.egg === 10){
                ...
                inventoryDisplay.style.zIndex = 5;
                playerStats.style.zIndex = 5;
                document.getElementById(`fullPlayerInfo`).style.zIndex = 5
                moveTo(afterEggs);
                player.companion = 2;
                for(let i = 0; i < 10; i++){
...
function changeBackgroundTo(nextPart){
    ...
    } else if((nextPart == wastelandDepartChad)|| (nextPart == wastelandDepartOwain)){
        background.style.backgroundImage = `url(/images/wastelands.jpeg)`
    } else if((nextPart == outpostChad) || (nextPart == outpostOwain) || (nextPart == leaveOutpostChad) || (nextPart == leaveOutpostOwain)){
        background.style.backgroundImage = `url(/images/wastelandOutpostCheckpoint.jpeg)`
    } else if((nextPart == pubOwain)|| (nextPart == pubChad)){
        background.style.backgroundImage = `url(/images/pubInterior.jpg)`
    } else if(nextPart == stoneTabletOwain){
...
function trueBossRandomness(action){
    if(action == `attack`){
        if(Math.random() > 0.3){
            return `attack`
        }else{
